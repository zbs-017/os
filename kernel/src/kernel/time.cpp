...
u8 Time::cmos_read(u8 addr)
{
    outb(CMOS_ADDR, CMOS_NMI | addr);
    return inb(CMOS_DATA);
};

void Time::time_read_bcd(Time* time)
{
...
extern "C" void time_init()
{
    Time time = Time();
    time.time_read(&time);
    Time::startup_time = time.mktime(&time);
