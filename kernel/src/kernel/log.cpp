...
int Log::printk(const char *fmt, ...) {
    ...
    this->console.write(buf, i);

    return i;
}

// 将字符数字串转换成整数，并将指针前移
int Log::skip_atoi(const char **s)
{
    int i = 0;
    while (is_digit(**s))
        i = i * 10 + *((*s)++) - '0';
...
// precision - 数字长度(精度)
// flags - 选项
char* Log::number(char *str, unsigned long num, int base, int size, int precision, int flags)
{
    char c, sign, tmp[36];
...
int Log::vsprintf(char *buf, const char *fmt, va_list args)
{
    int len;
    int i;
...
// 结果按格式输出字符串到 buf
int Log::sprintf(char *buf, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
