...
Console::Console() {
    this->attr = 7;        // 字符样式
    this->erase = 0x0720;  // 空格
}
Console::~Console() { }

void Console::get_screen() {
    outb(CRT_ADDR_REG, CRT_START_ADDR_H); // 开始位置高地址
    screen = inb(CRT_DATA_REG) << 8;      // 开始位置高八位
    outb(CRT_ADDR_REG, CRT_START_ADDR_L);
    screen |= inb(CRT_DATA_REG);

    screen <<= 1; // screen *= 2 一个字符由两个字节表示
    screen += MEM_BASE;
}

void Console::set_screen() {
    outb(CRT_ADDR_REG, CRT_START_ADDR_H); // 开始位置高地址
...
void Console::scroll_up() {
    ...
    {
        String::memcpy((void*)MEM_BASE, (void*)screen, SCR_SIZE);
        pos -= (screen - MEM_BASE);
        screen = MEM_BASE;
    }
    set_screen();
...
void Console::command_cr() {
    pos -= (x << 1);
    x = 0;
}

void Console::command_bs() {
    if (x)
    {
        x--;
        pos -= 2;
