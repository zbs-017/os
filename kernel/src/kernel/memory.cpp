...
extern "C" void memory_init(u32 magic, u32 addr) {
  ...
  PhysicalMemory::total_pages =
      IDX(PhysicalMemory::memory_size) + IDX(MEMORY_BASE);
...
extern "C" void memory_map_init() {
  ...
  PhysicalMemory::memory_map_pages =
      div_round_up(PhysicalMemory::total_pages, PAGE_SIZE);
  LOGK("Memory map page count %d\n", PhysicalMemory::memory_map_pages);

  PhysicalMemory::free_pages -= PhysicalMemory::memory_map_pages;

  // 清空物理内存数组
  String::memset((void *)PhysicalMemory::memory_map, 0,
                 PhysicalMemory::memory_map_pages * PAGE_SIZE);

  // 前 1M 的内存位置 以及 物理内存数组已占用的页，已被占用
  PhysicalMemory::start_page =
      IDX(MEMORY_BASE) + PhysicalMemory::memory_map_pages;
...
page_entry_t *KernelVirtualMemory::get_pde() { return this->pde; }

Bitmap *KernelVirtualMemory::get_kernel_map() { return &(this->kernel_map); }
